...
export default async function handler(req: NextApiRequest, res: NextApiResponse){
  ...
  try{
    ...
    const { data: rows, error } = await supabaseAdmin.from("mitarbeiter").select("id,nachname,pin_hash,aktiv,deleted_at").ilike("nachname",nachname.trim()).limit(1);
    if(error) throw error; const row = rows?.[0];
    if(!row||row.deleted_at||row.aktiv===false||!row.pin_hash) return res.status(401).json({ok:false,error:"Login fehlgeschlagen"});
    const ok = await bcrypt.compare(pin.trim(), row.pin_hash); if(!ok) return res.status(401).json({ok:false,error:"Login fehlgeschlagen"});
    const token = makeToken({ id: row.id, n: row.nachname, ts: Date.now() });
    res.setHeader("Set-Cookie", makeCookie("ma_sess", token, 60*60*12)); return res.status(200).json({ok:true});
